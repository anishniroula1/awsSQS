...
def update_output_container(input_year, selected_statistics):
    if selected_statistics == 'Recession Period Statistics':
        ...
        R_chart2 = dcc.Graph(
            figure=px.line(average_sales, x='Vehicle_Type', y='Automobile_Sales',
                           title='Average Number of Vehicles Sold by Vehicle Type')
        )

        exp_rec = recession_data.groupby('Vehicle_Type')['Total_Expenditure'].sum().reset_index()
        R_chart3 = dcc.Graph(
            figure=px.pie(exp_rec, values='Total_Expenditure', names='Vehicle_Type',
                          title='Total Expenditure Share by Vehicle Type during Recessions')
        )

        em_tysa = recession_data.groupby('Vehicle_Type').agg({'Unemployment_Rate': 'mean', 'Automobile_Sales': 'mean'}).reset_index()
        R_chart4 = dcc.Graph(
            figure=px.bar(em_tysa, x='Unemployment_Rate', y='Automobile_Sales', color='Vehicle_Type',
        ...
        Y_chart2 = dcc.Graph(
            figure=px.line(totmonsal, x='Month', y='Automobile_Sales', title='Total Monthly Automobile Sales')
        )
        ...
        Y_chart4 = dcc.Graph(
            figure=px.pie(exp_data, values='Advertisement_Expenditure', names='Vehicle_Type',
