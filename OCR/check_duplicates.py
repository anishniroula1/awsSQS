...
category_counts = filtered_df.groupby(['student_id', 'document_category']).size().reset_index(name='count')
...
total_duplicates = duplicates.groupby('student_id')['total_duplicates'].sum().reset_index()

# Display the total duplicates for each student_id
import ace_tools as tools; tools.display_dataframe_to_user(name="Total Duplicates by Student ID", dataframe=total_duplicates)
