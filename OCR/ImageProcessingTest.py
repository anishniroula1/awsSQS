...
def test_pdf_to_images(create_pdf):
    ...
    ), "Not all outputs are PIL images"
...
def test_rotate_image():
    ...
    rotated_image = ImageProcessing.rotate_image(
        original_image, 45
    )  # Rotate 45 degrees

    assert (
        rotated_image.shape[0] != original_image.shape[0]
    ), "Image height should change after rotation"
    assert (
        rotated_image.shape[1] != original_image.shape[1]
    ), "Image width should change after rotation"
...
def test_pdf_to_images_no_content():
    ...
    assert (
        len(images) == 0
    ), "Should not generate images from empty or invalid PDF content"
