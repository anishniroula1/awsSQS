...
def update_existing_documents(
    existing_documents: List[Document], content_list: List
) -> List[Dict]:
...
def find_new_documents(
    existing_documents: List[Document], content_list: List, existing_section_names: List
) -> List:
    """Identify new documents to add."""
    new_documents = [
        content
        for content in content_list
        if any(is_document_to_update(doc, content) for doc in existing_documents)
        or should_add_as_new_document(content, existing_section_names)
    ]
...
def get_list_of_new_and_existing_documents(
    ...
    exist_documents_to_update = update_existing_documents(
        existing_documents, content_list
    )
    new_documents = find_new_documents(
        existing_documents, content_list, existing_section_names
    )
