...
def retrieve_document_from_s3(document):
    if (
        document.get("id") is None
        or document.get("bucket_name") is None
        or document.get("s3_key") is None
    ):
        return None
    file_detail = {}
    try:
        aws = InterfaceAWS()
        s3_document = aws.get_object(
            document.get("bucket_name"), document.get("s3_key")
        )
