...
Exact match is performed under normalization:
- lowercase
- keep alphanumeric
- convert everything else to spaces
- collapse repeated spaces

This makes matching robust to case and punctuation differences.

Performance strategy:
- Build an anchor-token index (rarest token per phrase).
- For each sentence, only test phrases whose anchor token appears in the sentence.
"""
...
def normalize_with_map(s: str) -> Tuple[str, List[int]]:
    """Normalize text and build mapping from normalized index -> original index (for offsets)."""
    norm_chars: List[str] = []
    idx_map: List[int] = []
    prev_space = False
    ...
    if norm_chars and norm_chars[-1] == " ":
        norm_chars.pop(); idx_map.pop()
    return "".join(norm_chars), idx_map

def tokens(norm: str) -> List[str]:
    return TOKEN_RE.findall(norm)

def build_phrase_index_from_csv(csv_path: str, column_name: str = "sanction_list", logger: logging.Logger | None = None):
    """Load semicolon-separated phrases from CSV and build anchor_index for fast matching."""
    ...
    log.info("Sanctions index built: rows=%d raw_entities=%d unique=%d anchors=%d (%.2fs)",
             rows, raw_entities, len(phrases), len(anchor_index), time.time() - t0)
...
def find_exact_matches(sentence: str, phrases, anchor_index, max_candidates: int = 50_000) -> List[Dict[str, Any]]:
    """Find phrase matches in a sentence; max_candidates caps candidate phrases to avoid worst-case timeouts."""
